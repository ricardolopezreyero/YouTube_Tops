...
function scoreBaseSimple(video, channel) {
  ...
  const cr = views > 0 ? comments / views : 0;
  let eng  = Math.min(1, lr / 0.04) * 0.65 + Math.min(1, cr / 0.003) * 0.35;
  if (views > 5_000_000 && eng < 0.25) eng *= 0.5;

  const text  = `${video.title} ${video.description || ''}`.toLowerCase();
  const dHits = DEPTH_KEYWORDS.filter(k => text.includes(k.toLowerCase())).length;
  const depth = Math.min(1, dHits / 3) * 0.7 + (video.has_chapters === 1 ? 1 : 0) * 0.3;
...
async function main() {
  ...
  console.log(`   Semillas: ${seeds.length}`);
  ...
  for (let i = 0; i < seeds.length; i++) {
    const seed = seeds[i];
    process.stdout.write(`  [${i + 1}/${seeds.length}] "${seed.slice(0, 50)}"… `);
    try {
      ...
      for (const item of items) {
        ...
        if (!id) continue;
        if (!allVideoIds.has(id)) { allVideoIds.add(id); snippetMap[id] = item.snippet; queryMap[id] = seed; added++; }
      }
  ...
  if (allVideoIds.size === 0) { console.error('\n❌  No se obtuvieron videos.\n'); process.exit(1); }

  console.log(`\n  Enriqueciendo ${allVideoIds.size} videos…`);
  const enriched = await enrichVideos(YOUTUBE_API_KEY, [...allVideoIds]);
  console.log(`  ✓ ${enriched.length} videos`);

  enriched.forEach(item => { const id = item.snippet?.channelId; if (id) allChannelIds.add(id); });

  console.log(`\n  Canales: ${allChannelIds.size}…`);
  const channels = await enrichChannels(YOUTUBE_API_KEY, [...allChannelIds]);
  console.log(`  ✓ ${channels.length} canales`);

  const channelMap = {};
  channels.forEach(ch => { const fc = formatChannel(ch); channelMap[fc.channel_id] = fc; channelItems.push(fc); });
  ...
  console.log(`\n  Videos finales: ${videoItems.length} (${skipped} descartados)`);
  ...
  channelItems.forEach(c => sqlLines.push(channelSQL(c)));
  sqlLines.push('');
  videoItems.forEach(v => sqlLines.push(videoSQL(v)));

  const tmpFile = join(tmpdir(), `yt-tops-seed-${Date.now()}.sql`);
  writeFileSync(tmpFile, sqlLines.join('\n'), 'utf-8');

  console.log('\n  Ejecutando en D1 remoto…\n');
  ...
  if (result.status !== 0) {
    console.error('\n❌  Error al ejecutar en D1. Verifica wrangler login y el database_id.\n');
    process.exit(1);
  }

  console.log(`\n✅  Seed completado: ${videoItems.length} videos y ${channelItems.length} canales.\n`);
}
